...
{{
 ...
 "questions":[{{"q":"次に聞く一問を45文字以内","intent":"意図を30文字以内"}}],
...
questionsは【今の話題】に対する提案0〜3件。話題が変わったら前の質問を引き継がず、今に合うものだけを出す（純粋な雑談なら省略）。索引のdismissed（依頼主が却下した質問）と同趣旨のものは再提案しない。序盤の積み残し（open）を終盤に回収する質問を優先する。
...
def _normalize_fast_patch(patch):
    ...
    qs = [{"q": str(x.get("q") or "")[:45], "intent": str(x.get("intent") or "")[:30], "at": int(time.time())}
