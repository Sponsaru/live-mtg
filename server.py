...
# ライブASRは短いチャンクなので、ここを無制限に待つと1件の外部プロセス停止で
# 以後の文字起こしがすべて詰まる。長尺インポートは別枠で十分な時間を与える。
ASR_LIVE_TIMEOUT = max(30, int(os.environ.get("ASR_LIVE_TIMEOUT", "90")))
ASR_IMPORT_TIMEOUT = max(120, int(os.environ.get("ASR_IMPORT_TIMEOUT", "600")))
ASR_MAX_RETRIES = max(0, int(os.environ.get("ASR_MAX_RETRIES", "3")))
        ...
        try:
            ...
            if has_slides:
                ...
                with open(os.path.join(stage, _artifact_filename(sid, "mindmap_html", m)), "w", encoding="utf-8") as f:
                    f.write(txt)
            # プレゼン資料はPDFだけを外部成果物として届ける。slides.htmlは
            # PDF変換用の内部中間ファイルで、案件フォルダへは公開しない。
...
chunk_retry_counts = {}    # webm_path -> このプロセス中のASR失敗回数
chunk_retry_lock = threading.Lock()
...
def _asr_timeout(wav):
    """ライブ音声は素早く打ち切り、終了済み録音・モデル準備には長い猶予を与える。"""
    name = os.path.basename(str(wav or ""))
    is_live = ((name.startswith("inc_") and not name.startswith("inc_import_")) or
               name.startswith("prep_"))
    return ASR_LIVE_TIMEOUT if is_live else ASR_IMPORT_TIMEOUT
...
def _whisper_cpp(wav, sid=None):
    ...
    _run(["whisper-cli", "-m", MODEL, "-f", wav, "-l", _asr_language(sid), "-otxt", "-of", base,
          "--no-timestamps", "-np", "--prompt", _asr_hint(sid)], timeout=_asr_timeout(wav))
    txt = ""
    try:
        with open(base + ".txt", encoding="utf-8") as f:
            txt = f.read()
...
{{
 ...
 "conversationType":"今回の発話の主目的を固定8分類から1つ",
...
conversationTypeは、進捗・事実共有／背景説明・知識提供／質問・理解確認／課題・懸念の探索／アイデア・仮説の発散／意思決定・合意形成／実行計画・役割調整／関係構築・雑談、のどれか1つ。複数の要素があっても発話の主目的を1つ選ぶ。
...
def _merge_live_patch(old, patch, now):
    ...
    conversation_add = patch.get("conversationTypeAdd")
    if isinstance(conversation_add, dict):
        current_map = obj.get("conversationMap") if isinstance(obj.get("conversationMap"), dict) else {}
        current_types = list(current_map.get("types") or [])
        label = _conversation_type(conversation_add.get("type"))
        if label:
            target = next((row for row in current_types if _conversation_type(row.get("type")) == label), None)
            if target is None:
                target = {"type": label, "weight": 0, "topics": [], "examples": []}; current_types.append(target)
            target["weight"] = float(target.get("weight") or 0) + max(1, int(conversation_add.get("weight") or 1))
            topic = str(conversation_add.get("topic") or "").strip()[:42]
            example = str(conversation_add.get("example") or "").strip()[:100]
            if topic and topic not in [str(x.get("label") or "") for x in target.get("topics") or []]:
                target.setdefault("topics", []).append({"label": topic, "summary": ""})
            if example and example not in (target.get("examples") or []):
                target.setdefault("examples", []).append(example)
            obj["conversationMap"] = _conversation_map({"types": current_types}, "provisional", "character_volume")
...
- conversationType: 最新発話の主目的を次の固定8分類から必ず1つ。進捗・事実共有|背景説明・知識提供|質問・理解確認|課題・懸念の探索|アイデア・仮説の発散|意思決定・合意形成|実行計画・役割調整|関係構築・雑談
...
def _normalize_fast_patch(patch, delta=""):
    ...
    conversation_type = _conversation_type(patch.pop("conversationType", ""))
    if conversation_type:
        clean_delta = re.sub(r"\s+", " ", str(delta or "")).strip()
        patch["conversationTypeAdd"] = {"type": conversation_type,
                                        "weight": max(1, len(clean_delta)),
                                        "topic": str(patch.get("summary") or "").strip()[:42],
                                        "example": clean_delta[:100]}
    return patch
...
def _claude_update(sid):
    ...
    patch.pop("flow", None)
    patch = _normalize_fast_patch(patch, delta)
...
def process_chunk(sid, webm):
    ...
    try:
        ...
        # ここまで来れば音声のデコード・ASR・本文反映は完了。以下の派生解析が
        # 失敗しても同じ音声を再文字起こしして本文を重複させない。
        completed = True
        if txt:
            if not is_prep:
                try:
        ...
        # wav類は再生成できる一時物。受信webmはASRが正常完了した時だけ消す。
        # タイムアウト・例外時に残しておけば、キュー末尾への再試行と再起動復旧の
        # どちらからでも、原音を失わず追いつける。
        cleanup = [wav, os.path.splitext(wav)[0] + "-overlap.wav"]
        if completed:
            cleanup.insert(0, webm)
        for p in cleanup:
            try: os.remove(p)
            except Exception: pass
...
def _retry_failed_chunk(sid, webm, error):
    """失敗チャンクをキュー末尾へ回し、1件の停止で後続音声を塞がない。"""
    if not is_session(sid) or not os.path.isfile(webm):
        return False
    with chunk_retry_lock:
        attempts = chunk_retry_counts.get(webm, 0) + 1
        chunk_retry_counts[webm] = attempts
    if attempts > ASR_MAX_RETRIES:
        sys.stderr.write("[ASR] 再試行上限 %d回: %s（原音を保持し再起動時に復元）\n" %
                         (ASR_MAX_RETRIES, os.path.basename(webm)))
        sys.stderr.flush()
        return False
    sys.stderr.write("[ASR] %s: %s をキュー末尾へ再投入 (%d/%d)\n" %
                     ("タイムアウト" if isinstance(error, subprocess.TimeoutExpired) else "処理失敗",
                      os.path.basename(webm), attempts, ASR_MAX_RETRIES))
    sys.stderr.flush()
    enqueue_chunk(sid, webm)
    return True
...
def chunk_worker():
    ...
    while True:
        ...
        if failure is not None:
            _retry_failed_chunk(sid, webm, failure)
        else:
            with chunk_retry_lock:
                chunk_retry_counts.pop(webm, None)
...
<style id="livemtg-map-figure-capture">
  ...
  .mindmap-page .generated-radial.on { flex-direction: column !important; gap: .75rem !important; }
  .mindmap-page .generated-radial.on .generated-conversation-balance { width: min(1100px, 94%) !important; flex: none !important; }
  .mindmap-page .generated-radial.on .mermaid { height: auto !important; flex: 1 1 auto !important; min-height: 0 !important; }
...
def deck_is_current(sid):
    """清書・進行ボード・学び・文字起こしより新しいスライドPDFだけを完成扱いにする。"""
    folder = sdir(sid)
    deck = os.path.join(folder, "slides.pdf")
...
CONVERSATION_TYPE_LABELS = (
    "進捗・事実共有", "背景説明・知識提供", "質問・理解確認", "課題・懸念の探索",
    "アイデア・仮説の発散", "意思決定・合意形成", "実行計画・役割調整", "関係構築・雑談",
)

CONVERSATION_TYPE_ALIASES = {
    "進捗共有": "進捗・事実共有", "事実共有": "進捗・事実共有", "報告": "進捗・事実共有",
    "背景説明": "背景説明・知識提供", "知識提供": "背景説明・知識提供", "説明": "背景説明・知識提供",
    "質問": "質問・理解確認", "理解確認": "質問・理解確認", "確認": "質問・理解確認",
    "課題探索": "課題・懸念の探索", "懸念": "課題・懸念の探索", "問題提起": "課題・懸念の探索",
    "発散": "アイデア・仮説の発散", "アイデア": "アイデア・仮説の発散", "仮説": "アイデア・仮説の発散",
    "意思決定": "意思決定・合意形成", "合意形成": "意思決定・合意形成", "決定": "意思決定・合意形成",
    "実行計画": "実行計画・役割調整", "役割調整": "実行計画・役割調整", "段取り": "実行計画・役割調整",
    "関係構築": "関係構築・雑談", "雑談": "関係構築・雑談",
}

def _conversation_type(value):
    """AI表記揺れを固定8分類へ閉じ込める。未知分類は保存しない。"""
    text = re.sub(r"\s+", "", str(value or "")).strip()
    for label in CONVERSATION_TYPE_LABELS:
        if text == re.sub(r"\s+", "", label):
            return label
    for alias, label in CONVERSATION_TYPE_ALIASES.items():
        if alias in text:
            return label
    return ""

def _conversation_map(value, status="final", basis="ai_estimated_volume"):
    """会話タイプと比率を検証し、合計100%へ決定的に正規化する。"""
    source = value if isinstance(value, dict) else {}
    merged = {}
    for row in (source.get("types") or []):
        if not isinstance(row, dict):
            continue
        label = _conversation_type(row.get("type") or row.get("label"))
        if not label:
            continue
        target = merged.setdefault(label, {"type": label, "weight": 0.0, "topics": [], "examples": []})
        try:
            target["weight"] += max(0.0, float(row.get("weight", row.get("share", 0)) or 0))
        except (TypeError, ValueError):
            pass
        raw_topics = row.get("topics") or []
        for topic in raw_topics:
            if isinstance(topic, dict):
                item = {"label": str(topic.get("label") or topic.get("topic") or "").strip()[:42],
                        "summary": str(topic.get("summary") or topic.get("detail") or "").strip()[:120]}
            else:
                item = {"label": str(topic or "").strip()[:42], "summary": ""}
            if item["label"] and item not in target["topics"]:
                target["topics"].append(item)
        for example in (row.get("examples") or row.get("evidence") or []):
            text = str(example.get("text") if isinstance(example, dict) else example or "").strip()[:100]
            if text and text not in target["examples"]:
                target["examples"].append(text)
    rows = [merged[label] for label in CONVERSATION_TYPE_LABELS if label in merged and merged[label]["weight"] > 0]
    total = sum(row["weight"] for row in rows)
    if not total:
        return {"status": status, "basis": basis, "types": []}
    raw = [row["weight"] * 100.0 / total for row in rows]
    shares = [int(value) for value in raw]
    for index in sorted(range(len(rows)), key=lambda i: raw[i] - shares[i], reverse=True)[:100 - sum(shares)]:
        shares[index] += 1
    for row, share in zip(rows, shares):
        row["weight"] = round(row["weight"], 1)
        row["share"] = share
        row["topics"] = row["topics"][:4]
        row["examples"] = row["examples"][:3]
    rows.sort(key=lambda row: (-row["share"], CONVERSATION_TYPE_LABELS.index(row["type"])))
    return {"status": status, "basis": basis, "types": rows,
            "insights": [str(x).strip()[:120] for x in (source.get("insights") or []) if str(x).strip()][:3]}
...
{{
  ...
  "conversationMap": {{"types":[{{"type":"進捗・事実共有|背景説明・知識提供|質問・理解確認|課題・懸念の探索|アイデア・仮説の発散|意思決定・合意形成|実行計画・役割調整|関係構築・雑談","share":0,"topics":[{{"label":"実際の話題","summary":"このタイプとして交わされた内容"}}],"examples":["代表的な発言要旨"]}}],"insights":["会話配分から客観的に言える示唆"]}},
...
- conversationMap は「何を話したか」ではなく「どんな会話に時間を使ったか」の全体分析。固定8分類から実際に存在したものだけを出し、shareの合計を100にする。各発言を主目的で一度だけ数え、同じ内容を複数タイプへ重複計上しない。topicsは議題名の羅列ではなく、そのタイプとして行われた具体的な話題。examplesは文字起こしに根拠がある要旨だけ。insightsは「進捗共有に偏った」「意思決定が少ない」等、配分から客観的に言えることを最大3件。
...
def finalize_meeting(sid, hints="", speaker_map=None, corrections=None, source_signature=""):
    ...
    try:
        ...
        conversation_map = _conversation_map(obj.get("conversationMap"), "final", "ai_estimated_volume")
        if not conversation_map.get("types"):
            # モデルが欄を欠落させた場合も、ライブ中に全発話区間から積算した
            # 暫定分類を捨てず清書成果物へ引き継ぐ。
            for live_name in ("data.json", "data-live.json"):
                try:
                    with open(os.path.join(d, live_name), encoding="utf-8") as f:
                        live_snapshot = json.load(f)
                    conversation_map = _conversation_map(live_snapshot.get("conversationMap"), "final", "character_volume")
                    if conversation_map.get("types"):
                        break
                except Exception:
                    continue
        obj["conversationMap"] = conversation_map
...
class H(BaseHTTPRequestHandler):
    ...
    def _state(self):
        ...
        return {
            ...
            "hasDeckPdf": bool(current_id) and deck_is_current(current_id),
    ...
    def do_POST(self):
        ...
        if p == "/api/deck":
            ...
            return self._send(200, json.dumps(
                {"ok": True, "started": started, "busy": not started,
                 "url": "/deck.pdf?ts=%d" % int(time.time()),
        ...
        if p == "/api/finalize":
            ...
            if ok:
                # 清書で data.json の構造が確定した直後に静的マップも同期更新する。
                # 以前は既存mindmap.htmlが残り、再清書後も旧放射マップが表示された。
                map_ok, map_error = _ensure_mindmap_artifact(sid, read_meta(sid).get("title", "会議"))
                if not map_ok:
                    sys.stderr.write("[FINALIZE] mindmap refresh failed for %s: %s\n" % (sid, map_error)); sys.stderr.flush()
