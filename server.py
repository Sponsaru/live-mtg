        ...
        try:
            ...
            if has_slides:
                txt = (neutral_generated_html(sl, persist=True) or "").replace(
                    ...
                    'src="../../radial-map.js"', 'src="../radial-map.js"')
            ...
            if os.path.isfile(dk):
                txt = (neutral_generated_html(dk, persist=True) or "").replace(
                    'src="../../mermaid.min.js"', 'src="../mermaid.min.js"').replace(
                    'url("/brand-logo.png")', 'url("../brand-logo.png")')
            ...
            rj_src, rj_dst = os.path.join(SCRIPT_DIR, "radial-map.js"), os.path.join(base, "radial-map.js")
            if has_slides and os.path.isfile(rj_src):
                _sync_files(rj_src, rj_dst)   # レンダラは更新される可能性があるため毎回上書き
...
_NAME_STOP = {"お客", "皆", "お疲れ", "お母", "お父", "お兄", "お姉", "奥", "嫁", "婿", "患者", "店員"}
...
def _mech_confirms(sid, delta, transcript):
    ...
    for key, point, basis in cands:
        ...
        add.append({"point": point[:60], "basis": ("発話：" + basis)[:40]})
        if len(add) >= 2:
            break
    if not add:
        return
...
{{
 ...
 "arc":"会議開始からの流れを全体視点で3文以内に更新（既存arcを引き継ぎ、新しい展開を統合。話題の変遷と積み残しが分かるように）",
 ...
 "agenda_add":["新しく出た議題（体言止め）。無ければ空配列"],
 "points_add":["新しい論点・主張。無ければ空配列"],
 "todos_add":[{{"who":"担当（不明なら未定）","what":"やること"}}],
 "open_add":["未解決・要確認。無ければ空配列"]
}}
合意していないことをdecisionにしない。資料にない人名・会社名・数字を作らない。
relationは明確な因果・人物と話題・時系列がある時だけ。無ければ空オブジェクト。
questionは【会議の目標】と【会議全体の流れ(arc)】を踏まえた次の一手。目標が無くても会話を前進させる一問を提案してよい（純粋な雑談なら空オブジェクト）。序盤の積み残し（open）を終盤に回収する質問を優先する。
agenda_add/points_add/todos_add/open_addは各最大2件。既出（索引にあるもの）は再掲しない。
...
def _claude_update(sid):
    ...
    try:
        # 録音中はwhisper(MLX)がGPU/CPUを占有し、claude CLIの起動が15秒を超えることがある
        # （CLI単体は3〜4秒。2026-07-16実測：録音中だけFAST-ANALYSISが全滅→録音停止で回復）
        out = _ai_text(prompt, timeout=30, model=CLAUDE_MODEL)
    except Exception as e:
        fast_fail_streak += 1
...
def request_active_view_update(sid, force=False):
    ...
    # listは即時レーンが毎チャンク更新するようになった（2026-07-17 レーン統合＝AI呼び出し半減）
    if view in ("relation", "timeline", "list"): return
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/radial-map.js":
            return self._file(os.path.join(SCRIPT_DIR, "radial-map.js"), "application/javascript; charset=utf-8")
