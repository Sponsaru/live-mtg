...
- **匿名話者分離＋清書前確認（Mac・任意）**：`whispermlx` / pyannoteで音声をSpeaker A/Bへ分離。リアルタイム中の実名は候補表示に留め、清書前に発言例を見て名前を任意入力する。確認した対応だけを正式議事録へ反映し、未確認の名前は創作しない。Hugging Faceトークンとモデル利用規約への同意が必要。未設定・失敗時も通常の文字起こしと清書は継続する
