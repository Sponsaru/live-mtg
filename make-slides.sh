...
#   LiveMTG共通のニュートラルなHTMLフォーマットを使用する。
...
- LiveMTG表記とページ番号は自動で付くので出力しない。顧客・会社のロゴは絶対に追加しない。
- 下記の既定クラスだけを使う（新しいCSSクラスやstyle属性は作らない）。白・黒・グレー中心、強調のみ控えめなブルー。
...
10) 図形（Mermaid）: プロセス/相関/組織/時系列/対比が話に出たら図で見せる。stageの中に <div class="mermaid">…</div> を置く（Mermaid記法。配色は自動で共通ニュートラルテーマになる）。
   - フロー: flowchart LR
